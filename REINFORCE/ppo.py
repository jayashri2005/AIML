...
policy=ActroCritic(state_dim,action_dim)
...
for _ in range(epochs):
    ...
    r_t=new_probs[action]/action_probs[action]
    
    loss=-torch.min(r_t*advantage,torch.clamp(r_t,1-epsilon,1+epsilon)*advantage).mean()
